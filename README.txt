...
isn't a card...) and you SHOULD be fine, but there's still a small chance something else
might go wrong. I'll try to make sure that won't happen, mkay?
...
- Pygame 1.9.2a0  http://www.lfd.uci.edu/~gohlke/pythonlibs/#pygame
                    (Get the one that ends with win32-py2.7 and again keep clicking next)
- ...uh, there's probably something else I forgot,
     so you'll want to message me if yourun into any issues.

Optional, but RECOMMENDED:
- GIT 2.1.0       http://git-scm.com/downloads

Optional Stuff:
- PIL 1.1.7       http://effbot.org/downloads/PIL-1.1.7.win32-py2.7.exe (This is only if
                                                         you want to use the card editor)


==== USING GIT ====

Ok, so if you're reading this, it likely means you've already
downloaded everything, but still...
...
TO UPDATE THE PROJECT:
  Right-click the "TSSSFgame" folder and open 'GIT Bash'. Type in 'git pull' and it
  should update (or otherwise tell you everything is up to date).

TO UPDATE THE PROJECT WHEN IT WON'T LET YOU BECAUSE GIT'S A BUTT:
  Ok, something important to note here -
  THIS WILL REMOVE EVERYTHING YOU'VE CHANGED OR ADDED
  SO THINGS LIKE CUSTOM CARDS WILL DISAPPEAR!!
  So yeah, make sure to move them out of the folder first.
  OK anyways, just like the previous one, right-click the "TSSSFgame" folder and
  click 'open GIT Bash'. There's two commands for this part:
  'git fetch --all' followed by 'git reset --hard origin/master'.

I'll add more to this as time passes by.
